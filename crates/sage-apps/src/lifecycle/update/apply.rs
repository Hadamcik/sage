...
use crate::{
    ...
    fetch_recoverable_app_update, find_active_taskbar_runtime, find_runtime_by_app_id_optional,
...
pub(crate) async fn apply_recoverable_app_update_inner(
    ...
    let app_dir = std::path::PathBuf::from(recoverable.app_dir());
    let old_snapshot_dir = recoverable.active_snapshot_dir().to_string();
    let snapshot = prepare_update_snapshot(&app_dir, &pending).await?;
...
pub(crate) async fn apply_app_update_inner(
    ...
    if preflight.compatibility.ensure_installable().is_err() {
        ...
        let resolved = resolve_app(app_handle, app_id)
            ...
            .map_err(|resolve_err| {
                io::Error::other(format!(
                    "failed to read installed app {app_id}: {resolve_err}"
                ))
            })?;
    ...
    let compatibility_needs_review = matches!(
        ...
        crate::SageAppCompatibilityStatus::UntestedNewerSage { .. }
    );
...
async fn prepare_update_snapshot(
    app_dir: &std::path::Path,
    pending: &UserSageAppPendingUpdate,
) -> Result<crate::SageAppSnapshot> {
    ...
    write_snapshot_manifest(&snapshot)
        .map_err(|err| io::Error::other(format!("failed to write update manifest: {err}")))?;
...
mod tests {
    ...
    use super::{
        ensure_pending_update_matches_review, recovery_update_needs_review,
        resolve_updated_permissions,
    };
    use crate::{
