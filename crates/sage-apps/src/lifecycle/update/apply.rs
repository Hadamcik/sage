...
use crate::{
    ...
    SageAppView, SageGrantedPermissions, SageGrantedPermissionsInput, SharedSageApp,
    UserSageAppPendingUpdate, check_app_update_inner, download_url_snapshot,
    ...
    manager
        .mutate_shared_app(&app, move |ctx| {
            Box::pin(async move {
                ...
                if !granted_permissions_unchanged(
                    &expected_granted_permissions,
                    ctx.draft().app().common().granted_permissions(),
                ) {
                    anyhow::bail!(
...
fn granted_permissions_unchanged(
    expected: &SageGrantedPermissions,
    current: &SageGrantedPermissions,
) -> bool {
    current == expected
}
...
mod tests {
    ...
    use super::{
        SnapshotDirCleanup, ensure_pending_update_matches_review, granted_permissions_unchanged,
    };
    use crate::{SageGrantedPermissions, UserBridgeCapability};
    ...
    #[test]
    fn changed_permissions_abort_update() {
        let original = SageGrantedPermissions::default();
        let changed = SageGrantedPermissions::new_unchecked(
            [UserBridgeCapability::WalletGetVersion],
            [],
            Default::default(),
        );

        assert!(granted_permissions_unchanged(&original, &original));
        assert!(!granted_permissions_unchanged(&original, &changed));
    }
