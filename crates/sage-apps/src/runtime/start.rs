...
pub async fn create_runtime(
    ...
    let builder = WebviewBuilder::new(
        ...
        .on_navigation(move |url| {
            let runtime = runtime_for_nav.read();
            is_allowed_app_url(url, runtime.app())
        })
    ...
    if let Err(e) = add_child_result {
        let runtime = shared_runtime.read();
...
fn build_storage(mut builder: WebviewBuilder<Wry>, app: &SageApp)-> Result<WebviewBuilder<Wry>, String> {
    let has_persistent_storage = app
        .granted_permissions()
        .capabilities()
        .any(|cap| *cap == crate::bridge::capabilities::UserBridgeCapability::PersistentStorage);

    let should_use_incognito = !has_persistent_storage || app.flags().storage_may_contain_secrets();
